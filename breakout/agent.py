...
class DQNAgent:
    def __init__(self, config):
        ...
        self.batch_size = 32
        self.train_start = self.config.train_start # wait until this many episodes are recorded in memory
        self.update_target_rate = 10000 # how often to update the model
        self.discount_factor = 0.99
        self.memory = deque(maxlen=self.config.memory_size) # This is stored in RAM, can reduce memory usage by caching to disk
        self.no_op_steps = 30
        # build model
        ...
        self.optimizer = optim.RMSprop(self.model.parameters(), lr = self.config.lr, alpha = 0.95, eps = 0.01)
    ...
    def log_and_reset(self):
        ...
        np.save(os.path.join(self.config.logs_path, "q_log.npy"), np.array(self.q_log, dtype = np.float64))
        np.save(os.path.join(self.config.logs_path, "loss_log.npy"), np.array(self.loss_log, dtype = np.float64))
        np.save(os.path.join(self.config.logs_path, "unclipped_log.npy"), np.array(self.unclipped_log, dtype = np.float64))
        np.save(os.path.join(self.config.logs_path, "clipped_log.npy"), np.array(self.clipped_log, dtype = np.float64))
